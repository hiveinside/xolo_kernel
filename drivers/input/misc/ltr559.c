...
static void report_ps_input_event(struct ltr559_data *ltr559)
{
	...
	if(adc_value > ltr559->default_ps_highthresh)
	{
		input_report_abs(ltr559->ps_input_dev,
					ABS_DISTANCE, NEAR_VAL);
		input_sync(ltr559->ps_input_dev);
		ret = set_ps_range(ltr559->default_ps_lowthresh,0xFFF,LO_N_HI_LIMIT,ltr559);
	}
	else if(adc_value < ltr559->default_ps_lowthresh)
	{
		...
		input_report_abs(ltr559->ps_input_dev,
					ABS_DISTANCE, FAR_VAL);
		input_sync(ltr559->ps_input_dev);
		ret = set_ps_range(0X0,ltr559->default_ps_highthresh,LO_N_HI_LIMIT,ltr559);
	}
	if(ret < 0)
	{
		dev_err(&ltr559->i2c_client->dev,
...
static void set_ps_thelod(struct ltr559_data *ltr559)
{
	uint16_t ps_val;
	uint16_t sum = 0;
	uint16_t lh;
	...
	{
		msleep(10);
		ps_val = read_ps_adc_value(ltr559);
		sum += ps_val;
	}
	sum = sum/2;
	if(sum < 1799)
	{
		lh = sum + MIN_THD;
		hh = sum + MAX_THD;
		ltr559->default_ps_highthresh = hh;
		ltr559->default_ps_lowthresh = lh;
		
		rc = set_ps_range(0x0, ltr559->default_ps_highthresh,
			LO_N_HI_LIMIT, ltr559);
		if (rc < 0) {
			dev_err(&ltr559->i2c_client->dev,
				"%s : PS Thresholds Write Fail...\n", __func__);
