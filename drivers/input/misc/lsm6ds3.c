...
static int lsm6ds3_acc_gyr_hw_init(struct lsm6ds3_status *stat)
{
	...
	status_registers.ctrl1_xl.resume_val = 0x20;//modify by zhou
	//			status_registers.ctrl1_xl.default_val;
	status_registers.ctrl2_g.resume_val = 0x20;//modify by zhou
...
//		xyz[i] = (xyz[i] * GYR_RANG * GYR_HAL_COMPENSTATE) / GYR_REG_RANG;
	...
	//add by zhou set x y z orientation reverse.
	xyz[0] = -xyz[0];
	xyz[1] = -xyz[1];
	xyz[2] = -xyz[2];
                                        ...
                                        struct device* dev)
{
	...
	if (of_match_device(lsm6ds3_acc_gyr_dt_id, dev)) {
		...
		if (!of_property_read_u32(dn, "aa-filter-bw", &val)) {
			...
			stat->pdata_acc->fs_range = LSM6DS3_ACC_BW_400;
		}
